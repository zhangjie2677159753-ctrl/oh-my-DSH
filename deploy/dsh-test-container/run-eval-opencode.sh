...
MODEL="${DSH_TEST_MODEL:-deepseek-v4-flash}"
EVAL_OUT="${EVAL_OUT:-/tmp/omo-eval-ocg}"
...
run_scenario() {
  ...
  mkdir -p "$dir"
  local home
  home="$(DSH_TEST_MODEL="$MODEL" "$ROOT/deploy/dsh-test-container/prepare-home-opencode.sh")"
  local t0; t0=$(date +%s)
