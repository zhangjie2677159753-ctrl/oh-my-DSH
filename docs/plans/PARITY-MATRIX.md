...
| PAR-AGENT-004 | Metis gap analysis；agent 层 deny write/edit/apply_patch；运行时 tool-config-handler 对 librarian/explore/oracle/multimodal-looker/metis/momus 全部 task:deny | metis.ts、tool-config-handler.ts + tests | child structured result + profile-specific guard | no write；**运行时无 task**；Senpi 同禁 delegation；not primary | not-started |
| PAR-AGENT-005 | Momus plan review；agent 层 deny write/edit/apply_patch；运行时 task:deny（同 TASK_DENIED_SUBAGENT_KEYS）；Senpi one-shot/profile differs | momus.ts、tool-config-handler.ts + tests | saved-plan-path review + profile-specific guard | approve/reject；no write；**运行时无 task**；Senpi one-shot differential；not primary | not-started |
...
| PAR-PLAN-001 | Prometheus primary plan role；agent 源 map edit/bash/webfetch/question allow，但 tool-config-handler 运行时层 bash/interactive_bash:deny；`prometheus-md-only` 仅约束 Write/Edit 到 `.omo/*.md` 并给 delegation 加 warning | prompt/config/hook/tool-config-handler/tests | same Session planning phase + layered guard | Write/Edit path exact；**bash 运行时 deny**；delegation warning；no implementation before approval | not-started |
